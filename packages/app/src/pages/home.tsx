...
function HomeDesign() {
  ...
  function openPlasmaProject(conn: ServerConnection.Any, directory: string, initialize: boolean) {
    const ctx = global.createServerCtx(conn)
    ctx.projects.open(directory)
    ctx.projects.touch(directory)
    navigateOnServer(conn, `/${base64Encode(directory)}/plasma${initialize ? "?initialize=1" : ""}`)
  }
  ...
  async function choosePlasmaProject(initialize: boolean) {
    const conn = focusedServer()
    if (!conn) return
    const resolve = (result: string | string[] | null) => {
      const directory = Array.isArray(result) ? result[0] : result
      if (directory) openPlasmaProject(conn, directory, initialize)
    }
    const ctx = global.createServerCtx(conn)
    if (platform.openDirectoryPickerDialog && ctx.isLocal) {
      resolve(
        await platform.openDirectoryPickerDialog({
          title: initialize ? "Choose a folder for the new Solidity project" : "Open a Solidity project",
          multiple: false,
        }),
      )
      return
    ...
    dialog.show(
      () => <DialogSelectDirectory multiple={false} onSelect={resolve} server={conn} />,
      () => resolve(null),
    )
  }
    ...
    <div class="rounded-[10px] shadow-[var(--v2-elevation-raised)] m-2 bg-v2-background-bg-base self-stretch flex-1">
      <div class="mx-auto grid w-full h-full max-w-[1080px] gap-8 px-6 pb-16 lg:grid-cols-[280px_minmax(0,720px)]">
        <HomeProjectColumn
          ...
          openHelp={() => platform.openLink("https://github.com/CYPHES-ATP/Plasma#readme")}
          language={language}
        />

        <section class="min-w-0 flex-1 flex flex-col pt-12" aria-label={language.t("sidebar.project.recentSessions")}>
          <div class="mb-8 rounded-lg border border-border-weak-base bg-surface-base p-5">
            <div class="flex items-center gap-3">
              <PlasmaMark class="size-10 text-text-strong" />
              <div>
                <div class="text-18-medium text-text-strong">Plasma</div>
                <div class="text-13-regular text-text-weak">
                  AI-Powered Solidity with fingerprint-bound deployment gates.
                </div>
              </div>
            </div>
            <div class="mt-5 grid gap-2 sm:grid-cols-2">
              <Button class="justify-center" onClick={() => void choosePlasmaProject(true)}>
                Start New Contract
              </Button>
              <Button variant="ghost" class="justify-center" onClick={() => void choosePlasmaProject(false)}>
...
function LegacyHome() {
  ...
  async function choosePlasmaProject(initialize: boolean) {
    ...
    const resolve = (result: string | string[] | null) => {
      ...
      serverCtx.projects.open(directory)
      serverCtx.projects.touch(directory)
      navigate(`/${base64Encode(directory)}/plasma${initialize ? "?initialize=1" : ""}`)
    }
    if (platform.openDirectoryPickerDialog && server.isLocal()) {
      resolve(
        await platform.openDirectoryPickerDialog({
    ...
    dialog.show(
      () => <DialogSelectDirectory multiple={false} onSelect={resolve} server={s} />,
      () => resolve(null),
    )
  }
  ...
  return (
    <div class="mx-auto mt-55 w-full md:w-auto px-4">
      <div class="flex flex-col items-center text-center">
        <PlasmaMark class="size-20 text-text-strong" />
        <div class="mt-3 text-24-medium text-text-strong">Plasma</div>
        <div class="text-13-regular text-text-weak">AI-Powered Solidity</div>
        <div class="mt-5 grid w-full max-w-md gap-2 sm:grid-cols-2">
          <Button onClick={() => void choosePlasmaProject(true)}>Start New Contract</Button>
          <Button variant="ghost" onClick={() => void choosePlasmaProject(false)}>
            Audit Existing Project
          </Button>
        </div>
      </div>
