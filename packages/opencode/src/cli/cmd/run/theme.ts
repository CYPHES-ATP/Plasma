...
//
// Uses Plasma's dark theme for scrollback and footer colors while retaining
// the terminal palette for indexed splash colors.
import { RGBA, SyntaxStyle, type CliRenderer, type ColorInput, type TerminalColors } from "@opentui/core"
import type { TuiThemeCurrent } from "@opencode-ai/plugin/tui"
import type { EntryKind } from "./types"
import plasmaTheme from "../tui/context/theme/opencode.json" with { type: "json" }
...
export async function resolveRunTheme(renderer: CliRenderer): Promise<RunTheme> {
  try {
    ...
    const theme = resolveTheme(plasmaTheme as ThemeJson, "dark")
